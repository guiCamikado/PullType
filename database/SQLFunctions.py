...
class SQLFunctions:
    ...
    def create():
        cur = con.cursor()
        cur.execute("CREATE TABLE IF NOT EXISTS TABELA_CONTAS(user, password, email, email_verified, born_in, name, last_name, account_created_in, last_access, account_status, last_know_ip)")
        cur.close()
    create()
